...
void object_allocation_A(hugepage_info *a)
{
	...
	if(i%2 == 0)
	{
	    memset(a->hugepage_addr + i*32, 'A', 32);
	}
    }
}
...
void object_copy(void *a, void *b )
{
	...
	if(i%2 == 0)
	{
	    memcpy(b + i*32, a + i*32, 32);
	}
    }

    memset(a, 0, 2*1024*1024);
}
...
void object_allocation_B(hugepage_info *a)
{
	...
	if(i%2 == 1)
	{
	    memset(a->hugepage_addr + i*32, 'B', 32);
	}
    }
}
...
int main()
{
    ...
    for(int i=0; i<10; i++)
    {
	    ...
	    if(i%2==0)
	    {
	    hparray[i].hugepage_addr = mmap(NULL,HUGEPAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_HUGETLB | MAP_POPULATE, fds[i], 0);
...
//1
  ...
  void *addr_2 = mmap(addr_1, HUGEPAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_HUGETLB | MAP_FIXED, fds[1], 0);
    ...
    printf(" huge page 1 mapped at address_2: %p\n", addr_2);
    printf(" huge page 2 mapped at address for 2nd hugepage: %p\n", hparray[1].hugepage_addr);
 //2
...
//5
    ...
    printf("hugepage_2 page object \n");
    char *object_1;
    char *object_2;
    //hparray[0].hugepage_addr = addr_2;
    object_1 = (char*)hparray[2].hugepage_addr;
    object_2 = (char*)hparray[3].hugepage_addr;

    printf(" hugepage 1 starting vaddr: %p\n", hparray[2].hugepage_addr);

    printf(" hugepage 2 starting vaddr: %p\n", hparray[3].hugepage_addr);
